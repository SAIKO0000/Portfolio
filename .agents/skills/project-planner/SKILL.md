...
description: Generate and maintain the repository-root PLANS.md roadmap. Use when planning this project, updating roadmap priorities, recording decisions and risks, breaking work into verifiable tasks, or reconciling completed work with the plan.
---

# Project Planner Skill

## Repository-Local Contract (Highest Priority)

For this repository, this section overrides any generic output instructions later in this file.

- Maintain exactly one canonical roadmap at `<repository-root>/PLANS.md`.
- Do not create `requirements.md`, `design.md`, or `tasks.md` unless the user explicitly requests separate specifications.
- Before planning, inspect the existing `PLANS.md`, relevant source and documentation, dependency manifests, and `git status`.
- If `PLANS.md` does not exist, initialize it from the structure below. If it exists, update it in place; preserve useful decisions, completed history, and evidence.
- Keep plans repository-specific. Never insert generic phases, databases, APIs, or infrastructure that the code and product goals do not justify.
- Prefer the smallest viable architecture. Record a new service or dependency only when a concrete requirement warrants its operational cost.
- Every active task needs a verifiable outcome. Include dependencies only when they affect ordering.
- Reconcile the roadmap after meaningful implementation: move proven work to Completed, update risks and decisions, and keep Now focused.
...
## Product Direction
[Audience, portfolio promise, and success criteria.]

## Architecture Decisions
- **Decision:** Rationale and consequences.

## Now
- [ ] Outcome-oriented task — Verification: command or observable result.

## Next
- [ ] Prioritized follow-up.

## Later
- [ ] Optional or conditional enhancement.

## Risks and Open Questions
- Risk or decision still requiring evidence.

## Completed
- [x] Shipped outcome — Evidence: file, test, build, or date.
```

Keep `Now` short, order tasks by user value and dependency, and use `Later` for ideas that should not expand the current build. Do not estimate dates or effort unless the user asks.
