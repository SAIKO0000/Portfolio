...
description: "Primary layout and visual-design authority for this repository. Build or redesign polished browser-rendered visual artifacts with HTML/CSS/JavaScript/React: pages, dashboards, prototypes, animations, UI mockups, and data visualizations. Use for visual front-end creation, design-system exploration, design critique, or explicit browser acceptance / QA of a web artifact. Not for back-end, CLI, or non-visual coding."
---
...
- Give controls tactile feedback with subtle `transform` and `opacity` transitions; use small scale, translate, or dimensional cues instead of moving layout.
- Animate composited properties only. Avoid transitions on `width`, `height`, `top`, `left`, or `margin`.
- Use `will-change: transform` sparingly and only while an interaction is active; remove it after the animation completes.
- Gate cursor-following, magnetic, and elaborate hover effects behind `@media (hover: hover) and (pointer: fine)`.
- Respect `prefers-reduced-motion`; decorative motion must never be required to understand or operate the interface.
