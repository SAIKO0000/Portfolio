      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {mcpTools.map((tool, index) => (
          <AnimatedSection key={tool.name} delay={index + 1}>
            <button 
              className={`rounded-lg p-6 border transition-all duration-300 hover:scale-105 w-full text-left ${
                isDark 
                  ? 'bg-gray-700 border-gray-600 hover:bg-gray-650' 
                  : 'bg-white border-gray-200 hover:bg-gray-50'
              } ${selectedTool?.name === tool.name ? 'ring-2 ring-blue-500' : ''}`}
              onClick={() => runDemo(tool)}
              onKeyDown={(e) => e.key === 'Enter' && runDemo(tool)}
              aria-label={`Demo ${tool.name}`}
            >
              ...
              <div className="space-y-1">
                {tool.features.map((feature, idx) => (
                  <div key={`${tool.name}-feature-${idx}`} className="flex items-center text-xs">
